...
public class HyperlinkProviderImpl implements HyperlinkProviderExt {
    ...
    public void performClickAction(Document doc, int offset, HyperlinkType type) {
        ...
        List<Pair<LSPBindings, Location>> foundLocations = new ArrayList<>();
        Utils.handleBindings(LSPBindings.getBindings(file),
                             ...
                             (server, locations) -> {
                                 if (locations == null) {
                                    ...
                                    List<? extends Location> generalizedLocations;
                                    if (locations.isLeft()) {
                                        generalizedLocations = locations.getLeft();
                                     } else {
                                    ...
                                    generalizedLocations.stream()
                                                        .map(l -> Pair.of(server, l))
        ...
        if (foundLocations.isEmpty()) {
            ...
            Pair<LSPBindings, Location> location = foundLocations.get(0);
            Utils.open(location.first(), location.second().getUri(), location.second().getRange());
        } else {
            //TODO: show a popup
            Pair<LSPBindings, Location> location = foundLocations.get(0);
            Utils.open(location.first(), location.second().getUri(), location.second().getRange());
