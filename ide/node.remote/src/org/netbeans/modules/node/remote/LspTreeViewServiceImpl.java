 ...
 * under the License.
 */
package org.netbeans.modules.node.remote;
...
import org.netbeans.modules.node.remote.api.ConfigureExplorerParams;
...
import org.netbeans.modules.node.remote.api.ResourceData;
 ...
 */
@JsonSegment("nodes")
public class LspTreeViewServiceImpl implements TreeViewService {
    ...
    public CompletableFuture<TreeItem> explorerManager(CreateExplorerParams params) {
        System.err.println("explorer manager called!");
        try {
        String id = params.getExplorerId();
        return treeService.createProvider(id, params.getContext()).thenCompose(tv -> tv.getRootInfo()).toCompletableFuture();
        } catch (Throwable t) {
            t.printStackTrace();
            throw t;
        }
